...
export async function activate(context: vscode.ExtensionContext) {
    ...
    let activityTimer: NodeJS.Timeout | null = null; // Timer for general activities
    ...
    const onDidChangeTextEditorSelection = (e: vscode.TextEditorSelectionChangeEvent) => {
        const now = Date.now();
    ...
    const onDidStartDebugSession = (session: vscode.DebugSession) => {
    ...
    const onDidTerminateDebugSession = (session: vscode.DebugSession) => {
        const file = vscode.window.activeTextEditor?.document.fileName ?? 'N/A';
    ...
    const onDidOpenTerminal = (terminal: vscode.Terminal) => {
    ...
    const onDidCloseTerminal = (terminal: vscode.Terminal) => {
    ...
    const onDidChangeTerminalState = (terminal: vscode.Terminal) => {
        const now = Date.now();
